...
namespace System.Data.Entity.SqlServer.SqlGen
{
    ...
    internal class SqlGenerator : DbExpressionVisitor<ISqlFragment>
    {
        ...
        public override ISqlFragment Visit(DbLikeExpression e)
        {
            ...
            System.Data.Entity.Core.Common.CommandTrees.DbConstantExpression Pattern = e.Pattern as System.Data.Entity.Core.Common.CommandTrees.DbConstantExpression;
            if(Pattern!=null && Pattern.Value !=null && Pattern.Value.ToString().IndexOf("%nu|") == 0){
                result.Append(" dbo.App_fnConvertString(");
                result.Append(e.Argument.Accept(this));
                result.Append(") LIKE ");
                if (Pattern.Value.ToString().IndexOf("~%%") > 0)
                {
                    var r = Pattern.Value.ToString().Replace("%nu|", "") ;
                    r = r.Replace("~%%", "%");
                ...
                {
                    result.Append("'" + Pattern.Value.ToString().Replace("%nu|", "%") + "'");
                }
            }
            else{
                result.Append(e.Argument.Accept(this));
