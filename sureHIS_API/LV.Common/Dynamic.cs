...
namespace LV.Common
{
    public static class DynamicQueryable
    {
        ...
        public static IQueryable<TResult> Join<TResult>(this IQueryable outer, IEnumerable inner, string outerSelector, string innerSelector, string resultsSelector, params object[] values)
        {
            if (inner == null) throw new ArgumentNullException("inner");
            if (outerSelector == null) throw new ArgumentNullException("outerSelector");
            if (innerSelector == null) throw new ArgumentNullException("innerSelector");
            ...
            ParameterExpression[] parameters = new ParameterExpression[] {
            Expression.Parameter(outer.ElementType, "outer"), Expression.Parameter(inner.AsQueryable().ElementType, "inner") };
            LambdaExpression resultsSelectorLambda = DynamicExpression.ParseLambda(parameters, typeof(TResult), resultsSelector, values);

            return outer.Provider.CreateQuery<TResult>(
                Expression.Call(
                    typeof(Queryable), "Join",
                    new Type[] { outer.ElementType, inner.AsQueryable().ElementType, outerSelectorLambda.Body.Type, typeof(TResult) },
                    outer.Expression, inner.AsQueryable().Expression, Expression.Quote(outerSelectorLambda), Expression.Quote(innerSelectorLambda), Expression.Quote(resultsSelectorLambda)));
        }

        public static IQueryable GroupJoin(this IQueryable outer, IEnumerable inner, string outerKeySelector, string innerKeySelector, string resultSelector, params object[] values)
        {
        ...
        //public static IEnumerable<T> SycnQuery<T>(this IQueryable<T> query, DbContext dbContext) where T : class
        //{
        //    if (dbContext == null) return query;
        //    IEnumerable<T> list = query.ToList();

        //    list = list.Except(dbContext.ChangeTracker.Entries<T>().Where(x => x.State == EntityState.Deleted).Select(x => x.Entity));
        //    list = list.Union(dbContext.ChangeTracker.Entries<T>().Where(x => x.State == EntityState.Added).Select(x => x.Entity));
        ...
        {
#if DB2
            predicate = "DB2" + predicate;
            if(values[0] is Guid)
                values = new object[] { ((Guid)values[0]).ToByteArray() };
            else if(values[0] is bool)
                values = new object[] { (((bool)values[0]) ? (Int16)1 : (Int16)0) };
#endif
        ...
        //public static string ToStringSQL(this IQueryable source)
        //{
